...
const App: React.FC = () => {
  ...
  const handleImageUpload = useCallback(async (filesToProcess: UploadedFilePayload[]) => {
    ...
    // Append new files to existing processedInvoices, or replace if that's the desired behavior.
    // For now, let's replace to show only current batch results.
    // If you want to append: setProcessedInvoices(prev => [...prev, ...initialStates]);
    ...
    // Update the states with processing results
     setProcessedInvoices(prevResults => {
        return prevResults.map(pr => {
            const matchingResult = results.find(r => r.id === pr.id);
            return matchingResult ? matchingResult : pr;
        });
    });
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    return () => {
      processedInvoices.forEach(invoice => {
        // Only revoke if it's an object URL created by FileReader and not a data URL
      ...
      <footer className="mt-24 mb-10 w-full max-w-4xl mx-auto px-6 text-sm text-slate-600 border-t border-slate-300 pt-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
          <div className="md:col-span-1">
            ...
            <p>&copy; {new Date().getFullYear()} TxTechs.</p>
            <p>Giải pháp hóa đơn thông minh.</p>
          </div>

          <div className="md:col-span-1">
            <h3 className="text-md font-semibold text-slate-700 mb-3">Giải Pháp AI Của TxTechs</h3>
            <p className="leading-relaxed">
              Chuyên cung cấp các giải pháp AI tiên tiến: OCR hóa đơn, Nhận diện khuôn mặt (Face Recognition), Đếm người (People Counting), Phân tích AI trong bán lẻ (AI in Retail), Phát hiện đối tượng (Object Detection), Computer Vision, Heatmap Instore Analytics, Traffic Monitor, Chatbot, AI Agents và các ứng dụng AI tùy chỉnh khác.
            </p>
          </div>
            ...
            <ul className="space-y-2">
              <li>
                <a href="tel:+84898336308" className="inline-flex items-center hover:text-blue-600 transition-colors group">
                  ...
                  <span>Phone: +84 898 336 308</span>
                </a>
              </li>
              <li>
